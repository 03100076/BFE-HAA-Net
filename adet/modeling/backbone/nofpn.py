...
class SingleNOFPN(Backbone):
    ...
    def __init__(
        ...
        """
        super(SingleNOFPN, self).__init__()

        self.out_channels = out_channels
        # build 5-levels nofpn
        ...
        for fnode in self.nodes:
            ...
            in_channels = node_info[feat_level]
            output_conv = Conv2d(
                in_channels,
                out_channels,
                kernel_size=1,
            ...
            self.add_module(
                "output_f{}".format(feat_level), output_conv
            )
    ...
    def forward(self, feats):
        ...
        for fnode in self.nodes:
            feat_level = fnode["feat_level"]
            input_node = feats[feat_level]

            name = "output_f{}".format(feat_level)
            new_feats.append(self.__getattr__(name)(input_node))

        return new_feats


class NOFPN(Backbone):
    """
    This module implements Feature Pyramid Network.
    It creates pyramid features built on top of some input feature maps.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            num_repeats (int): the number of repeats of NoFPN.
            norm (str): the normalization to use.
        """
        ...
        # build nofpn
        self.repeated_nofpn = nn.ModuleList()
        for i in range(num_repeats):
            ...
            self.repeated_nofpn.append(SingleNOFPN(
    ...
    def forward(self, x):
        ...
        for nofpn in self.repeated_nofpn:
             feats = nofpn(feats)
...
def build_fcos_resnet_nofpn_backbone(cfg, input_shape: ShapeSpec):
    """
    Args:
    ...
    if cfg.MODEL.MOBILENET:
        ...
        bottom_up = build_resnet_backbone(cfg, input_shape) # choose this branch
    in_features = cfg.MODEL.NOFPN.IN_FEATURES # ['res3', 'res4', 'res5'] 
    out_channels = cfg.MODEL.NOFPN.OUT_CHANNELS # 256?
    num_repeats = cfg.MODEL.NOFPN.NUM_REPEATS # 1?
    top_levels = cfg.MODEL.FCOS.TOP_LEVELS # 0 or 2
