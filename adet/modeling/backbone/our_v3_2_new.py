...
# from .haa import HAANet, SpaceAtt, ChannelAtt
from .haa import ChannelAttAvg as ChannelAtt
# from .haa import ChannelAttMax as ChannelAtt
# from .haa import ChannelAttCat as ChannelAtt
# from .haa import ChannelAttCatFirst as ChannelAtt
...
class SingleBiFPN_v3_2(Backbone):
    """
    ...
    def __init__(
        ...
        """
        super(SingleBiFPN_v3_2, self).__init__()

        self.out_channels = out_channels

        self.nodes = [ {'feat_level': 2, 'inputs_offsets': [2, 3]},
                        {'feat_level': 1, 'inputs_offsets': [1, 2, 4]},
                        {'feat_level': 0, 'inputs_offsets': [0, 1, 5]},
                        {'feat_level': 1, 'inputs_offsets': [5, 6]},
                        {'feat_level': 2, 'inputs_offsets': [4, 5, 7]},
                        {'feat_level': 3, 'inputs_offsets': [3, 4, 8]}]
        ...
        for fnode in self.nodes:
            ...
            ## add ChannelAtt
            if top_down_flag:
                name = "ChannelAtt_f{}".format(feat_level)
                self.add_module(name, ChannelAtt(out_channels))
            if feat_level == 0:
                top_down_flag = False
    ...
    def forward(self, feats):
        ...
        for fnode in self.nodes:
            ...
            ## ChannelAtt forward: feature update
            if feat_level == 0:
                curr_num = len(feats)
                for level in range(num_levels):
                    name = "ChannelAtt_f{}".format(level)
                    if hasattr(self, name):
                        feats[curr_num - 1 - level] = self.__getattr__(name)(feats[curr_num - 1 - level])
...
class BiFPN_v3_2(Backbone):
    ...
    def __init__(
        self, bottom_up, in_features, out_channels, num_top_levels, num_repeats, norm="", post_process=None
    ):
        """
        Args:
            bottom_up (Backbone): module representing the bottom up subnetwork.
            ...
            norm (str): the normalization to use.
        """
        super(BiFPN_v3_2, self).__init__()
        ...
        for i in range(num_repeats):
            ...
            self.repeated_bifpn.append(SingleBiFPN_v3_2(
                in_channels_list, out_channels, norm
            ))
        
        # build post_process
        if post_process:
            for idx, fea_name in enumerate(self._out_features):
                if idx == 0:
                    continue#filter p2
                name = "post_{}".format(fea_name)
                self.add_module(name, ChannelAtt(out_channels))
    ...
    def forward(self, x):
        ...
        # post_process
        for ind in range(len(self._out_features)): 
            name = self._out_features[ind]
            layer_name = "post_{}".format(name)
            if hasattr(self, layer_name):
                feats[ind] = self.__getattr__(layer_name)(feats[ind])
...
def build_fcos_resnet_bifpn_backbone_v3_2(cfg, input_shape: ShapeSpec):
    """
    Args:
    ...
    in_features = cfg.MODEL.BiFPN.IN_FEATURES # ['res2', 'res3', 'res4', 'res5']
    out_channels = cfg.MODEL.BiFPN.OUT_CHANNELS
    num_repeats = cfg.MODEL.BiFPN.NUM_REPEATS
    top_levels = cfg.MODEL.FCOS.TOP_LEVELS # 记得设置为 0
